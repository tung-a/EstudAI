      ...
      try {
        ...
        if (stored) {
          ...
          if (parsed?.conversations?.length) {
            ...
            setConversationCounter(parsed.counter ?? parsed.conversations.length + 1);
            return;
          }
        }
  ...
  useEffect(() => {
    ...
    const persistState = async () => {
      try {
        ...
        await AsyncStorage.setItem(CONVERSATIONS_STORAGE_KEY, JSON.stringify(payload));
      } catch (error) {
  ...
  const parseSuggestedQuestions = (raw: string): string[] => {
    ...
    rawLines.forEach((line) => {
      ...
      if (bulletMatch || aggregated.length === 0) {
        ...
        aggregated[aggregated.length - 1] = `${aggregated[aggregated.length - 1]} ${cleanedLine}`.trim();
      }
    });
...
Resposta do assistente: "${answer}".`,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const followUps = await getFollowUpSuggestions(trimmedInput, responseText);
        ...
        <View style={styles.layout}>
          ...
          >
            ...
            >
              ...
              {sidebarExpanded ? (
                <Text
                  style={[styles.sidebarToggleText, { color: themeColors.text }]}
                >
                  Recolher
                </Text>
              ) : null}
                ...
                return (
                  ...
                  >
                    <TouchableOpacity
                      ...
                      onLongPress={() => confirmDeleteConversation(conversation.id, conversation.title)}
                      delayLongPress={250}
                      style={[
                    ...
                    >
                      ...
                      {sidebarExpanded ? (
                        <Text
                          numberOfLines={1}
                          style={[styles.conversationItemText, { color: textColor }]}
                        >
                          {conversation.title}
                        </Text>
                      ) : null}
                    ...
                    {sidebarExpanded ? (
                      <TouchableOpacity
                        onPress={() => confirmDeleteConversation(conversation.id, conversation.title)}
                        style={[
            ...
            >
              ...
              {sidebarExpanded ? (
                <Text
                  style={[styles.newConversationButtonText, { color: themeColors.text }]}
                >
                  Nova conversa
                </Text>
              ) : null}
